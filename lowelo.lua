...
function AbilitySpam5:FindPlayer()
    local char = LocalPlayer.Character
	...
	local target10 = playersName[1] or nil
	return target10
end

function AbilitySpam5:GetPlayerCFrame()
    local p = self:FindPlayer()
			...
			pcall(function()
				...
				for i=1,2 do
					...
					local args = {
						...
						9000000,
						{
							HitboxCFrames = {targetCF,targetCF,targetCF,targetCF,targetCF,targetCF},
							BestHitCharacter = targetChar,
							HitCharacters = {targetChar,targetChar,targetChar},
							Ignore = {},
							DeathInfo = {},
							BlockedCharacters = {},
							HitInfo = {
...
function AbilitySpam5:Teammatekill()
    if self.connection then return end
    self.enabled = true
	teammateplayer = AbilitySpam5:FindNearestPlayer()
...
button.MouseButton1Click:Connect(function()
    ...
    updateButton()
    if enabled then
		pcall(function() AbilitySpam5:Start()  end)
	else
		pcall(function() AbilitySpam5:Stop()  end)
    end
end)
...
updateButton()
task.wait(0.2)
pcall(function() AbilitySpam5:Teammatekill()  end)
